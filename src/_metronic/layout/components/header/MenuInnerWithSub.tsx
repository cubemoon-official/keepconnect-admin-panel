...
const MenuInnerWithSub: React.FC<Props & WithChildren> = ({
  ...
  // ✅ Only intercept click if onClick exists
  const handleClick = (e: React.MouseEvent<HTMLSpanElement, MouseEvent>) => {
    if (onClick) {
      e.preventDefault() // prevent navigation if modal
      onClick()
    } else if (to) {
      navigate(to) // normal route navigation
    }
  }
    ...
    <div ref={menuItemRef} className='menu-item menu-lg-down-accordion me-lg-1'>
      <span
        className={clsx('menu-link py-3', {
          active: to ? checkIsActive(pathname, to) : false,
        })}
        onClick={onClick || to ? handleClick : undefined} // attach click only if needed
